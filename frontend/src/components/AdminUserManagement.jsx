...
function AdminUserManagement() {
  ...
  const handleDeleteUser = async (user) => {
    ...
    const confirmed = window.confirm(`Delete user "${user.username || user.email || user._id}"? This action cannot be undone.`);
    if (!confirmed) return;

    setDeleting(prev => ({ ...prev, [user._id]: true }));
    try {
      await axios.delete(`${API_URL}/admin/users/${user._id}`, { withCredentials: true });
...
// change user roles for customize admin login

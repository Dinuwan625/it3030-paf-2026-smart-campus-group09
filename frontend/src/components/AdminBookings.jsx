...
function AdminBookings() {
  ...
  useEffect(() => {
    ...
    if (role !== 'ADMIN' && role !== 'TECHNICIAN') { navigate('/'); return; }
  ...
  const handleDeleteBooking = async (bookingId) => {
    ...
    try {
      const res = await axios.delete(`${API_URL}/bookings/${bookingId}`, { withCredentials: true });
      if (res.data?.success) {
        setBookings(prev => prev.filter(b => b._id !== bookingId));
        showToast('Booking deleted successfully');
      } else {
        showToast(res.data?.message || 'Failed to delete booking.', true);
      }
    } catch (err) {
      showToast(err.response?.data?.message || 'Failed to delete booking.', true);
    } finally {
      setDeleting(null);
    }
  };
                  ...
                  {filtered.map(b => (
                    <tr key={b._id}>
                      ...
                      <td style={{ maxWidth: 180 }}>
                        <span title={b.purpose} style={{ display: 'block', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 165 }}>
                          {b.purpose}
                        </span>
                      </td>
                      ...
                      <td style={{ maxWidth: 220 }}>
                        <span
                          ...
                          style={{ display: 'block', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 205 }}
                        >
...
// change booking types for admin
